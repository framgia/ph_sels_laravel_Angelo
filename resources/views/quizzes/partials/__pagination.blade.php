                    ...
                    <span class="relative z-0 inline-flex shadow-sm rounded-md">
                        ...
                        <div class="self-center">
                            <p class="text-sm text-gray-700 leading-5 px-3">
                                ...
                                {!! __('of') !!}
                                <span class="font-medium">{{ $words->total() }}</span>
                            </p>
                        </div>

                        {{-- Next Page Link --}}
                        @if ($words->hasMorePages())
                            <a href="{{ $words->nextPageUrl() }}" rel="next" class="relative inline-flex items-center px-2 py-2 -ml-px text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-r-md leading-5 hover:text-gray-400 focus:z-10 focus:outline-none focus:ring ring-gray-300 focus:border-blue-300 active:bg-gray-100 active:text-gray-500 transition ease-in-out duration-150" aria-label="{{ __('pagination.next') }}">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd" />
                                </svg>
                            </a>
                        @else
                            <a class="btn bg-green-600 text-white" href="{{ route('quizResult', ['quiz' => $quiz->id]) }}">
                                {{ __('Finish') }} 
                            </a
                        @endif
                    </span>
